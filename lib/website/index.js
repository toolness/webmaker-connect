...
exports.express = function(app, options) {
  ...
  app.get('/docs/api/', function(req, res, next) {
    ...
    return res.render('docs/api.html', {
      ...
      renderNotificationEmail: function(options) {
        var sendOptions = api.notificationEmail({
          application: {name: options.appName},
          user: {username: options.username, email: options.email},
          text: options.text,
          namedRoutes: app.namedRoutes
        });
        sendOptions = email.normalizeSendOptions(sendOptions);
        return email.sendOptionsToPlainText(sendOptions);
      }
    });
  });
