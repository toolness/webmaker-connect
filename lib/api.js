...
function notificationEmail(options) {
  ...
  var appName = options.application.name;
  var username = options.user.username;
  ...
  return {
    ...
    text: nunjucksEnv.render('notification-email.txt', template.safeContext({
      appName: appName,
      username: username,
      indentedText: options.text.split('\n').map(function(line) {
        return '    ' + line.trim()
      }).join('\n'),
      unsubscribeURL: options.namedRoutes.reverseAbsolute('app:authorized')
    }))
  };
}
...
exports.express = function(app, options) {
  ...
  app.post('/api/account/notify.json', [
    ...
    emailBackend.send(notificationEmail({
      nunjucksEnv: app.nunjucksEnv,
      application: req.oauthToken.application,
      user: req.oauthToken.user,
      text: req.body.text,
      namedRoutes: app.namedRoutes
    }), function(err) {
      if (err) return next(err);
