...
exports.express = function(app, options) {
  ...
  app.all('/api/oauth/access_token', [
    ...
    var accessToken = new Tokens.AccessToken({
      userInfo: requestToken.userInfo,
      application: requestToken.application._id
    });

    accessToken.save(function(err) {
      if (err) return next(err);

      Tokens.RequestToken.findByIdAndRemove(requestToken._id, function(err) {
        if (err) return next(err);
  ...
  app.get('/api/account/settings.json', [
    ...
    return res.send(req.oauthToken.userInfo);
  });
};
