...
function hmacSignedMiddleware(options) {
  ...
  return function(req, res, next) {
    ...
    var isUsingRequestToken = isAccessTokenReq;
    var isUsingAccessToken = !(isRequestTokenReq || isAccessTokenReq);
    var requestToken = isUsingRequestToken ? oauth.oauth_token : null;
    ...
    getSecrets({
      ...
      if (!secrets || !secrets.consumer ||
          ...
          (isUsingRequestToken && !secrets.requestToken))
        return res.send(401, 'invalid consumer key or request/access token');
